---
title: 'Pipeline multilenguaje (Python, R, Julia) para el monitoreo de manglar en la CGSM (2013--2025) — versión compacta'
...
Los inventarios globales basados en Landsat consolidaron una narrativa en la que la pérdida de manglar obedece en su mayor parte a la presión antrópica directa: Goldberg et al. (2020), mediante Random Forest sobre más de un millón de escenas Landsat a 30 m, atribuyen el 62 % de la pérdida mundial entre 2000 y 2016 al cambio de uso del suelo ---principalmente acuicultura y agricultura de exportación, con casi el 80 % en seis países del sudeste asiático---; este marco, centrado en transiciones abruptas tipo estanques o cultivos, tiende a invisibilizar las pérdidas graduales que dominan otras geografías y su aplicación al contexto colombiano resulta limitada. Murillo-Sandoval et al. (2022), al reconstruir 36 años de cobertura con LandTrendr, documentan una disminución de aproximadamente 48 000 ha ---14 % del área nacional--- y muestran que la transición dominante no es la conversión abrupta sino la degradación de manglar denso a otra vegetación (38 469 ± 2 829 ha), con retroceso sostenido en el Pacífico desde 2004 y descensos en el Caribe entre 1984--1988 y posteriores a 2012, asociados a expansión agrícola, vías y alteraciones hidrosedimentarias. La tensión entre ambas fuentes ---global frente a nacional--- sustenta la necesidad de aproximaciones por trayectorias temporales densas, capaces de capturar transiciones intermedias entre cobertura plena y suelo desnudo.
...
Entre los antecedentes específicos para el complejo lagunar resulta pertinente el trabajo de Vinasco et al. (2020), quienes cuantificaron los cambios de cobertura en la CGSM entre 2013 y 2018 a partir de 119 escenas Landsat-8 procesadas de manera local, con corrección DOS y clasificación supervisada mediante Random Forest y perceptrón multicapa entrenados sobre 300 muestras distribuidas en seis clases CORINE Land Cover; articularon el análisis con NDVI, EVI y NDWI como descriptores temporales y reportaron un incremento de la frontera agrícola y urbana con contracción de las áreas húmedas del 27 % al 12 %. Esta aproximación, afín al presente estudio en el uso de índices espectrales sobre Landsat, opera como ejercicio de clasificación de coberturas generales y no aísla el manglar, de manera que el aporte de este trabajo consiste en sustituir los clasificadores supervisados por umbrales NDVI y CMRI sobre series armonizadas Landsat 8/9 y Sentinel-2 y en migrar el procesamiento íntegramente a Google Earth Engine.
...
![Arquitectura general del proyecto: cinco fuentes de datos heterogéneas (Sentinel-2/Landsat, ERA5/CHIRPS, ENSO NOAA, caudal IDEAM e INVEMAR/GBIF) convergen en un datacube unificado en NetCDF CF-1.8 sobre EPSG:9377, sobre el cual operan tres lenguajes con responsabilidades complementarias ---Python para adquisición e inteligencia artificial, R para estadística y BFAST, Julia para fragmentación---, produciendo cinco productos abiertos que materializan la reproducibilidad.](../outputs/figures/arquitectura_datos.png){#fig-arquitectura width=95% fig-pos="H"}

El proyecto se desarrolla siguiendo un pipeline modular y reproducible cuya arquitectura general se sintetiza en la figura @fig-arquitectura, organizada en cuatro fases y un modulo de validacion que se ejecutan dentro de un contenedor Docker ---sig_unal v1.11, que integra Python 3.12, R 4.3.3, Julia 1.11.3 y Quarto 1.4---, de modo que se garantiza la replicabilidad completa del entorno de análisis. Cada fase se implementa aprovechando las fortalezas del lenguaje mas adecuado para la tarea, y la interoperabilidad entre fases se mantiene mediante formatos estandar ---GeoTIFF, GeoJSON y CSV---.
...
La lectura posterior de los cubos se realiza con `xarray.open_dataset(chunks=...)` aplicando evaluación perezosa: las dimensiones (`time`, `x`, `y`) y los atributos CF-1.8 ---`Conventions`, `institution`, `creator_name`, `crs_origen`--- se decodifican automáticamente y solo se materializa el resultado final con `.compute()`.
...
```python
# Z-scores y detección de anomalias
...
Los datos se reciben en formato NetCDF con convenciones CF y se cargan con `xarray.open_dataset(chunks=...)` aplicando evaluación perezosa, decodificando automáticamente las unidades originales ---metros por día para precipitación, kelvin para temperatura---. Sobre la serie se calcula la climatología mensual y la anomalía respecto a esa climatología, se promedia espacialmente sobre el AOI acotado y se cruza con la anomalía NDVI promediada sobre las ocho estaciones para rezagos de 0, 1, 2 y 3 meses, de modo que se identifique el lag óptimo entre forzante y respuesta del manglar.
...
El análisis de series temporales de NDVI sobre las 8 estaciones reveló 18 anomalías significativas (z < -2) durante 2013--2025, cuya distribución se sintetiza en la tabla @tbl-anomalias. El evento de septiembre de 2020 emerge como la perturbación de mayor magnitud, con valores negativos de NDVI en Punta Cerro e Isla Boquerón, coincidente con el inicio de La Niña 2020--2021 que provocó inundaciones prolongadas en el sistema lagunar. Un segundo cluster se concentra en marzo de 2018 sobre las estaciones occidentales ---Caño Clarín y CP Aguas Negras---, probablemente asociado a hipersalinización en época seca, en tanto que la extensión de la serie con Landsat 8 revela 4 anomalías adicionales en VIPIS durante 2016 que no eran visibles con la serie Sentinel-2 sola, lo cual demuestra la importancia de contar con series temporales largas. Las estaciones con mayor variabilidad fueron Caño Palos y Caño Clarín (std ≈ 0,145), mientras que Punta Chino mostró la menor (std = 0,078), consistente con su ubicación protegida en el borde suroriental. Con base en estos hallazgos se seleccionaron tres periodos de referencia para la segmentación ---degradación (julio--diciembre 2020), recuperación (enero--junio 2022) y estado actual (julio 2024 a junio 2025)---, definidos bajo una lógica de muestreo de estados estables antes que de serie continua: cada ventana de seis a doce meses representa una fase fenológica posterior a la disipación de la perturbación previa, de modo que el composite resultante refleja un estado estructural y no un transitorio. El periodo "actual" cubre un único año hidrológico para capturar el estado vigente al cierre del proyecto sin contaminar el composite con La Niña 2023--2024, así sus resultados deben interpretarse como un snapshot y no como tendencia anualizada, coherente con la lógica de Bunting et al. (2022) sobre épocas discretas.
...
Conviene aclarar el alcance temporal de los tres periodos: la **degradación** (julio--diciembre 2020) captura la respuesta espectral durante la mortandad asociada a La Niña; la **recuperación** (enero--junio 2022) corresponde a la regeneración temprana documentada por INVEMAR (2024); y el periodo **actual** (julio 2024--junio 2025) constituye un *snapshot* del estado más reciente al momento de la entrega y no una tendencia post-recuperación estabilizada, de modo que la comparación entre los tres debe leerse como secuencia de estados puntuales ---se requieren al menos dos ciclos anuales adicionales (2025--2027) para diferenciar contracción estable de fluctuación intermedia---. Esta consideración matiza las inferencias sobre cambio neto que se discuten a continuación.

Conviene precisar que la contracción del área clasificada por umbrales no equivale, por sí sola, a pérdida de cobertura del manglar; en efecto, la figura @fig-ndvi-cambio ---calculada directamente sobre el datacube trimestral CF-1.8 como diferencia de NDVI mediano entre el periodo actual y el de degradación--- muestra que la mayor parte del AOI acotado presenta tonos azules consistentes con **ganancia de vigor vegetativo** sobre los parches sobrevivientes, mientras los tonos rojos se concentran en los bordes hipersalinos del Vía Parque y en sectores puntuales del Santuario asociados a las lagunas internas. Ambos hallazgos son compatibles: el manglar se consolida en menos parches pero más densos espectralmente, así la métrica de área filtrada subestima la dinámica del ecosistema cuando se interpreta de forma aislada, de manera que se justifica reportar conjuntamente las métricas de fragmentación, las series temporales y el mapa de cambio NDVI, pues los tres se sostienen mutuamente.
...
Esta evidencia debe interpretarse con cautela por tres motivos convergentes: la resolución de ERA5-Land ---nueve kilómetros nominales--- promedia la precipitación sobre una zona que excede la escala característica del sistema lagunar y no captura microclimas locales; el mecanismo causal real de la mortandad de septiembre 2020 no es la lluvia caída sobre la CGSM sino el incremento de caudal del río Magdalena ---y secundariamente Sevilla, Aracataca y Fundación--- desde cuencas altas en la Sierra Nevada, donde la lluvia La Niña sí fue copiosa, así el insumo apropiado son las series IDEAM y no ERA5-Land sobre el píxel del humedal; y el contraste de signos opuestos entre manglar y estaciones limnológicas confirma que el promedio de las ocho estaciones ---aplicado en la versión preliminar--- enmascara una señal real, lo que justifica retrospectivamente la clasificación por naturaleza espectral previa a cualquier correlación.
...
La cadena causal que estos datos obligan a reformular se articula así: el episodio La Niña 2020--2021 induce un régimen prolongado de mayor precipitación regional sobre la cuenca del Magdalena y la Sierra Nevada que se acumula en los caudales fluviales hasta alcanzar valores extremos en 2022; el aporte sostenido de caudal es beneficioso para el manglar en términos agregados ---reduce la hipersalinización del sustrato y favorece la productividad del dosel---, lo cual explica las correlaciones positivas con NDVI; el evento puntual de mortandad de septiembre 2020 no se explica por un pico de caudal simultáneo ---que no existe en las series IDEAM--- sino por la lluvia local intensa sobre el píxel del humedal ($\rho_{\text{ERA5}} = -0{,}123$ a rezago de dos meses, tabla @tbl-era5) acompañada de inundación bajo dosel detectable por SAR (43,08 km², tabla @tbl-sar); y los efectos de mediano plazo del régimen La Niña se manifiestan en los quiebres bfast de 2022 sobre CP Aguas Negras y CP Luna (tabla @tbl-bfast-unificado), periodo en el cual los caudales alcanzan su máximo histórico. Esta reformulación honra los datos sin renunciar a la atribución climática del evento de 2020 a La Niña, en el entendido de que la mediación entre el forzante atmosférico global y la respuesta espectral local opera a través de procesos hidrológicos multiescalares que ningún forzante único captura por sí solo.
...
Conviene precisar el alcance de la validación en tres frentes. La métrica reportada se calcula sobre la clasificación por umbrales espectrales con restricciones geofísicas y no sobre la salida directa de SamGeo, cuya segmentación automática cubre cualquier objeto coherente del raster RGB ---incluidos agua y suelo desnudo--- y requiere filtrado posterior por CMRI o NDVI medio, paso que en esta entrega se realiza implícitamente al evaluar la clasificación por umbrales como producto. WorldCover identifica aproximadamente 412 km² de manglar dentro del AOI mientras el clasificador identifica 229 km², es decir, una subestimación sistemática atribuible al umbral conservador NDVI > 0,70 que sugiere bajarlo a 0,60--0,65 o complementarlo con CMRI para mejorar la Recall sin comprometer la Precision. La diferencia residual entre ambas cartografías corresponde a heterogeneidad espectral del manglar de la CGSM ---vegetación riberana, pastizales inundables, salitrales--- y a la distancia entre el clasificador supervisado de WorldCover (entrenado globalmente sobre etiquetas) y la regla determinística por umbrales del proyecto, distinción pertinente al comparar este F1 con los rangos 0,80--0,90 de Selvaraj y Gallego-Pérez (2023) para flujos óptico-SAR con Random Forest.
...
Se generó un dashboard interactivo en formato HTML autocontenido mediante el script `src/python/make_dashboard_html.py`, que construye el mapa con folium y Earth Engine sirviendo las capas como teselas mediante mapId. La versión vigente del dashboard opera sobre el AOI acotado leído del archivo `data/raw/cgsm_aoi_acotado_4326.geojson` e integra 17 capas temáticas organizadas en cuatro bloques. El bloque de **estado del manglar** comprende el NDVI por periodo de referencia, el mapa de cambio NDVI (Actual − Degradación) y la clasificación de manglar por periodo (degradación, recuperación, actual). El bloque de **dinámica de cobertura** comprende las capas de ganancia, pérdida y estabilidad de manglar entre 2020 y 2024--2025. El bloque de **inundación** comprende la detección SAR de septiembre--octubre 2020 en agua abierta y bajo dosel junto con la frecuencia histórica de inundación de la Global Flood Database 2001--2017 ---un mapa de calor amarillo a púrpura que cuenta el número de eventos GFD que afectó cada píxel del AOI---. El bloque de **referencia y contexto** comprende el polígono del área de estudio, las estaciones de monitoreo separadas por naturaleza espectral y la cartografía oficial de manglares de Colombia a escala 1:25.000 (INVEMAR, 2020) cargada como vectorial con tooltip interactivo que muestra el área en hectáreas, el tipo de cobertura y el departamento de cada polígono. El dashboard se abre directamente en cualquier navegador web sin necesidad de Jupyter ni de software especializado, así constituye una herramienta de consulta accesible para gestores ambientales del Plan de Manejo Ambiental del sitio Ramsar; sus mapId Earth Engine tienen vigencia limitada de algunas horas, de modo que conviene regenerar el HTML poco antes de cada sesión de presentación. La versión preliminar del dashboard sobre el AOI envolvente, generada con `geemap` y dependiente de un kernel Jupyter activo (HTML de 2,7 MB con widgets ipyleaflet embebidos), se conserva en el notebook legacy `06_dashboard.ipynb` para trazabilidad metodológica.

# Conclusiones

El pipeline multilenguaje desarrollado caracterizó la dinámica espaciotemporal de la cobertura de manglar en la CGSM durante 2013--2025, integrando series temporales NDVI en Python (929 registros mensuales Landsat 8 + Sentinel-2), detección de quiebres con bfast en R, segmentación automática con SamGeo, métricas de fragmentación en Julia y validación cruzada con SAR y cartografía de referencia. Sobre el AOI acotado (SFF CGSM + VPI Salamanca, 835,3 km²) bfast detecta un quiebre estructural generalizado en 2016 ---7 de 8 estaciones, asociado a El Niño 2015--2016--- y la perturbación de septiembre de 2020 ---z < −3 en Isla Boquerón y Punta Cerro, asociada a La Niña 2020--2021---, refinada por el análisis bfast unificado sobre las cuatro estaciones de manglar denso que confirma quiebres en 2020 (Caño Palos junio; Caño Clarín febrero y diciembre), en 2022 (CP Aguas Negras abril; CP Luna enero) y un tercer bloque en 2023--2024 coincidente con El Niño 2023--2024 visible en la serie ONI. La serie combinada arroja 18 anomalías significativas (z < −2), de las cuales 4 sobre VIPIS en 2016 solo emergen al extender con Landsat 8. La fragmentación describe un patrón de **contracción del área clasificada con consolidación estructural**: los parches pasan de 79 a 15, el área total cae de 12.425,6 a 4.037,0 ha y el área media sube de 157,3 a 269,1 ha entre degradación y estado actual, en tanto que MSI aumenta de 0,51 a 1,46 y NND de 1,10 a 2,39 km. Paralelamente, la diferencia de NDVI 2020 vs 2024--2025 (figura @fig-ndvi-cambio) y la serie de NDVI mediano del manglar (figura @fig-ndvi-manglar) ---que se recupera de 0,60 en 2020 a valores estables cercanos a 0,80 desde 2022--- evidencian una **ganancia generalizada de vigor vegetativo**, con un cambio neto positivo de +76,0 km² entre ambos periodos. El SAR Sentinel-1 diferencia dos mecanismos durante septiembre--octubre 2020: 15,93 km² de agua abierta y 43,08 km² bajo dosel, para 59,02 km² afectados (7,1 % del AOI), complementados por la dinámica hídrica JRC 1984--2021 con pérdida de 18,55 km² de agua permanente y ganancia compensatoria de 23,55 km², de modo que el sistema redistribuye su lámina más que la pierde. La validación doble contra INVEMAR 1:25.000 (Instituto de Investigaciones Marinas y Costeras, 2020) y ESA WorldCover v200 (Zanaga et al., 2022) arroja F1-scores convergentes de **0,583** y **0,548** ---mejora del 24--32 % sobre el F1 = 0,442 del baseline envolvente---, con Precision entre 0,768 y 0,811 y Specificity entre 0,944 y 0,954; el acuerdo entre ambas referencias (F1 = 0,833) define el techo metodológico realista, de manera que el clasificador alcanza el 70 % de ese techo. Frente a la literatura, la cobertura reportada es coherente con el rango GMW v3.0 para 2018--2020 (Bunting et al., 2022), si bien el F1 = 0,548 queda por debajo del rango 0,80--0,90 de Selvaraj y Gallego-Pérez (2023) en el Pacífico colombiano, lo cual se atribuye al uso de umbrales espectrales sin entrenamiento supervisado y a la mayor heterogeneidad espectral del complejo lagunar.

Entre las limitaciones técnicas figuran el remuestreo obligado de 10 a 30 m por la RAM de SamGeo vit_b ---que reduce la resolución de la segmentación---, la discontinuidad espectral Landsat 8 ↔ Sentinel-2 alrededor de 2018 ---visible en VIPIS (NDVI medio L8 = 0,090 vs S2 = 0,353)--- que puede sesgar bfast, la Recall sostenida en 0,43--0,45 por el umbral conservador NDVI > 0,70, y la aproximación previa del cálculo de áreas en Julia mediante coordenadas geográficas (sustituida por EPSG:9377 en la versión vigente). Como trabajo futuro inmediato se proponen cinco líneas: ampliar la red IDEAM más allá de El Banco y Ganadería Caribe ---ya validadas por CHIRPS sobre las mismas cuencas (tablas @tbl-caudal-corr y @tbl-chirps-corr)--- incorporando ríos Fundación, Río Frío y tributarios secundarios; profundizar el análisis ENSO ya iniciado con ONI y SOI (tabla @tbl-enso-corr) mediante métricas de eventos discretos e índices complementarios como AMO; refinar el clasificador hacia el techo metodológico de F1 = 0,83 ajustando el umbral NDVI a 0,60 o entrenando Random Forest con muestras INVEMAR; extender el bfast unificado al periodo 2013--2025 incorporando Landsat 8 sobre las cuatro estaciones de manglar denso; y ampliar la ventana del periodo "actual" en al menos dos ciclos anuales (2025--2027) para diferenciar fluctuación intermedia de tendencia estable. Pese a estas limitaciones, el pipeline demuestra la viabilidad de un enfoque GeoAI multilenguaje para el monitoreo costero y constituye un prototipo reproducible adaptable a otros sistemas lagunares tropicales; el dashboard interactivo se ofrece como insumo cartográfico para el seguimiento permanente que establece el Plan de Manejo Ambiental del sitio Ramsar CGSM (Comisión Conjunta CGSM, 2026).
...
- `notebooks/08_validacion_multilingual.ipynb`: Comparación series Python+GEE v