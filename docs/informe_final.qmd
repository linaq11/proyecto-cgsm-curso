...
Este trabajo utiliza datos de observación terrestre de la **NASA-USGS** (Landsat 8/9, SRTM v3), de la **NOAA** (índices climáticos ENSO ONI y SOI del Climate Prediction Center) y del **Dartmouth Flood Observatory** (Global Flood Database, derivada de MODIS NASA y Sentinel-1 ESA). La segmentación promptable del manglar se realiza con la biblioteca **SamGeo** (Wu y Osco, 2023), adaptación geoespacial del *Segment Anything Model* de Meta AI desarrollada por **Qiusheng Wu** (OpenGeos). Se agradece a INVEMAR por el acceso a la cartografía de referencia 1:25.000 y al IDEAM por los registros hidrométricos del río Magdalena.
