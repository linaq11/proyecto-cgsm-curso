...
- El pipeline aporta una capa cuantitativa al monitoreo del Plan de Manejo Ambiental del sitio Ramsar, relevante para las comunidades pesqueras de Tasajera, Pueblo Viejo y Buenavista, cuya subsistencia depende de la salud del manglar (protección costera, sostenimiento de la pesca artesanal).
:::
...
La cobertura de manglar de la CGSM sostiene servicios ecosistémicos críticos para las comunidades pesqueras del corredor Tasajera-Pueblo Viejo-Buenavista: provisión de hábitat para pesquerías comerciales (jaiba, mojarra, sábalo), protección costera frente a marejadas y eventos extremos, y captura de carbono azul. La mortandad del manglar de septiembre 2020 ---documentada en este informe mediante 43,08 km² de inundación bajo dosel detectada por Sentinel-1 SAR--- coincidió temporalmente con afectaciones reportadas a la pesca artesanal regional en el marco del episodio La Niña 2020-2021. En este contexto, el módulo de alertas tempranas Digital Twin Nivel 2 desarrollado en este trabajo se sitúa dentro del paradigma de **early warning humanitario** asociado a fenómenos hidroclimáticos en humedales costeros tropicales: la combinación de detección retrospectiva (bfast), inventario histórico (Global Flood Database), forzamiento climático (ENSO) y vigilancia operacional mensual (NDVI + SAR) ofrece una herramienta cuantitativa para anticipar perturbaciones que afectan tanto a la biodiversidad protegida como a las comunidades dependientes del ecosistema.

# Agradecimientos

Este trabajo utiliza datos de observación terrestre de la **NASA-USGS** (Landsat 8/9, SRTM v3) y de la **NOAA** (índices climáticos ENSO ONI y SOI del Climate Prediction Center), así como el inventario histórico de inundaciones del **Dartmouth Flood Observatory** (Global Flood Database, derivado de MODIS NASA y Sentinel-1 ESA). La segmentación promptable del manglar se realiza con la biblioteca **SamGeo** (Wu y Osco, 2023), adaptación geoespacial del modelo *Segment Anything Model* de Meta AI desarrollada por el **Prof. Qiusheng Wu** (OpenGeos), cuya participación virtual como ponente en la conferencia **NASA Lifelines** ([nasalifelines.org](https://nasalifelines.org/)) motivó el énfasis de este proyecto en el uso de datos NASA aplicados a temas de **acción humanitaria** ---específicamente a la línea de *Flood Early Warning*--- mediante SIG y sensores remotos. Se agradece adicionalmente a INVEMAR por el acceso a la cartografía de referencia 1:25.000 y al IDEAM por los registros hidrométricos del río Magdalena.
