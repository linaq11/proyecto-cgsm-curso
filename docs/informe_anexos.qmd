---
title: 'Anexos técnicos del Pipeline multilenguaje para el monitoreo de manglar en la CGSM (2013--2025)'
subtitle: 'Documento complementario del informe final'
...
- **Anexo D** reporta el análisis de sensibilidad del acoplamiento caudal-NDVI con la variable alternativa de caudal máximo mensual.
- **Anexo E** contiene los fragmentos de código que materializan cada fase del pipeline.
- **Anexo F** reúne dos tablas de detalle (cruce SAR-NDVI-GFD por estación y transiciones JRC) referenciadas desde el cuerpo del informe.
...
# Anexo D: Análisis de sensibilidad — caudal máximo vs medio mensual de El Banco

El cuerpo del informe usa el caudal **medio mensual** del río Magdalena en El Banco por coherencia con Ganadería Caribe (Aracataca, disponible solo en variable media). El análisis paralelo con la variable alternativa **caudal máximo mensual** ---también disponible en DHIME, 144 obs. 2013--2025--- arroja correlaciones positivas universales con NDVI cuyo signo coincide y cuyo pico también se ubica en el rezago de tres meses (manglar $\rho_{\text{máx}} = +0{,}277$ vs $\rho_{\text{medio}} = +0{,}256$). La diferencia operativa se concentra en los rezagos cortos: $\rho_{\text{máx}}$ supera a $\rho_{\text{medio}}$ por 0,08--0,12 unidades en lag 0--1, consistente con que los picos extremos del caudal máximo se asocian más rápidamente con cambios espectrales puntuales mientras el caudal medio captura el efecto agregado del régimen sostenido. Las conclusiones científicas del cuerpo del informe son por tanto robustas a la elección de la variable hidrométrica; los datos detallados de ambas series están disponibles en `outputs/tables/caudal_comparacion_max_medio.csv`.
...
: Cruce de señal SAR, NDVI y frecuencia de inundación histórica por estación, restringido al AOI acotado SFF + VPI Salamanca. {#tbl-cruce tbl-colwidths="[12, 12, 12, 10, 12, 11, 31]"}
...
: Transiciones JRC Global Surface Water (1984--2021) dentro del AOI acotado SFF + VPI Salamanca. El AOI contiene 385,3 km² de agua permanente (>75 % del tiempo) y 44,1 km² estacional (25--75 %), de modo que casi la mitad del polígono protegido es lámina de agua. {#tbl-jrc tbl-colwidths="[40, 18, 42]"}
...
: Entorno de ejecución completo del proyecto. {#tbl-entorno}
...
: Quiebres estructurales detectados por bfast sobre las ocho estaciones del proyecto en la serie combinada Landsat 8 + Sentinel-2 (2013--2025). El análisis refinado sobre las cuatro estaciones de manglar denso se reporta en la tabla @tbl-bfast-unificado del informe principal. {#tbl-bfast}
...
: Parches de manglar truncados por la frontera del AOI acotado (predicado DE-9IM `intersects` con tolerancia de 30 m). Los parches se cuentan agregados como features completos en `geopandas`. {#tbl-topología-acotado}
...
: Anomalias significativas de NDVI ($z < -2$) en la serie combinada 2013--2025, ordenadas por z-score decreciente. {#tbl-anomalias}
...
: Eventos de inundación con mayor área afectada dentro del AOI acotado (Global Flood Database, 2001-2017). {#tbl-gfd tbl-colwidths="[30, 22, 22, 16, 10]"}
