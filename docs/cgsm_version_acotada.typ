...
#table(
  ...
  table.header([Forzante], [Rezago óptimo (meses)], [$rho$ vs NDVI manglar],),
  table.hline(),
...
position: top, 
[
Correlación de Pearson entre cada forzante climático-hidrológico y la anomalía NDVI z-score sobre las cuatro estaciones de manglar denso.
]), 
kind: "quarto-float-tbl", 
supplement: "Tabla", 
)
...
El hallazgo metodológico más significativo del estudio es el contraste de signos opuestos entre la precipitación ERA5-Land local ($rho = - 0 \, 123$) y los forzantes regionales (caudal $rho = + 0 \, 256$, CHIRPS $rho = + 0 \, 252$, SOI $rho = + 0 \, 146$) sobre las estaciones de manglar. Esta aparente contradicción no es inconsistencia metodológica sino evidencia del carácter no lineal de la respuesta del manglar al forzamiento climático: el ENSO global y el caudal de la cuenca alta organizan el régimen hídrico de fondo ---beneficioso al contrarrestar la hipersalinización crónica que el INVEMAR #cite(<invemar2024>, form: "year") identifica como principal estresor histórico---, mientras la precipitación local sobre el píxel del humedal captura la intensidad puntual del aporte hídrico, así eventos de lluvia copiosa y prolongada se asocian con inundación bajo dosel e hipoxia que sí afectan negativamente al dosel. La cadena causal La Niña → caudal fluvial → inundación prolongada → mortandad opera, en consecuencia, a través de variables intermedias multiescalares que ningún forzante único captura por sí solo, lo que justifica la convergencia entre cuatro fuentes independientes como herramienta de triangulación.
